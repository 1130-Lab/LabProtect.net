...
namespace AntiCrack_DotNet
{
    public sealed class OtherChecks
    {
        ...
        public static bool IsInvokedAssembly(bool DetectHooks)
        {
            ...
            bool IsEntryAssemblyNotNull = EntryAssembly != null;
            string ExecutablePath = Process.GetCurrentProcess().MainModule.FileName;
            if (EntryAssembly != null)
            {
                if (ExecutingAssembly.Location != EntryAssembly.Location)
                    return true;
                if (ExecutingAssembly.GetName().Name != EntryAssembly.GetName().Name)
                    return true;
