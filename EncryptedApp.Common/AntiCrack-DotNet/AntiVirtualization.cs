...
namespace AntiCrack_DotNet
{
    internal sealed class AntiVirtualization
    {
        ...
        private static HashSet<string> BadFileNames = new HashSet<string> { "Vboxmouse.sys", "vboxguest.sys", "vboxsf.sys", "vboxvideo.sys", "vmmouse.sys", "vboxogl.dll" };
        private static HashSet<string> BadDirs = new HashSet<string> { @"C:\Program Files\VMware", @"C:\Program Files\oracle\virtualbox guest additions" };
        private static bool _isBadVMFileDetectionInitialized = false;
        private static volatile bool _badVMFileNameDetected = false;
        public static bool BadVMFilesDetection()
        {
            if (!_isBadVMFileDetectionInitialized)
            {
                var badFileNameWatcher = new FileSystemWatcher(Environment.GetFolderPath(Environment.SpecialFolder.System));
                badFileNameWatcher.NotifyFilter = NotifyFilters.FileName | NotifyFilters.DirectoryName | NotifyFilters.LastWrite;
                badFileNameWatcher.Created += (s, e) =>
                {
                    _badVMFileNameDetected = BadFileNames.Contains(e.Name!);
                };
                badFileNameWatcher.Changed += (s, e) =>
                {
                    _badVMFileNameDetected = BadFileNames.Contains(e.Name!);
                };
                badFileNameWatcher.Renamed += (s, e) =>
                {
                    _badVMFileNameDetected = BadFileNames.Contains(e.Name!);
                };
                badFileNameWatcher.EnableRaisingEvents = true;

                var badDirWatcher = new FileSystemWatcher("C://Program Files//");
                badDirWatcher.NotifyFilter = NotifyFilters.DirectoryName | NotifyFilters.LastWrite;
                badDirWatcher.Created += (s, e) =>
                {
                    _badVMFileNameDetected = BadDirs.Contains(e.FullPath.ToLower());
                };
                badDirWatcher.Changed += (s, e) =>
                {
                    _badVMFileNameDetected = BadDirs.Contains(e.FullPath.ToLower());
                };
                badDirWatcher.Renamed += (s, e) =>
                {
                    _badVMFileNameDetected = BadDirs.Contains(e.FullPath.ToLower());
                };
                badDirWatcher.EnableRaisingEvents = true;

                HashSet<string> fileNames = Directory.GetFiles(Environment.GetFolderPath(Environment.SpecialFolder.System)).Select(t => t.ToLower()).ToHashSet();
                foreach (string System32File in fileNames)
                {
                    try
                    {
                        if (BadFileNames.Contains(System32File))
                        {
                            return true;
                        }
                    }
                ...
                _isBadVMFileDetectionInitialized = true;
            }
            return _badVMFileNameDetected;
        }
